...
class Subject(models.Model):
    ...
    image = models.ImageField(upload_to=save_subject_image, blank=False, verbose_name='Subject Image')
    description = models.TextField(max_length=500, blank=True)
...
def save_lesson_files(instance, filename):
    ...
    if instance.lesson_id:
        filename = 'lesson_files/{}.{}'.format(instance.lesson_id, ext)
...
class SlotSubject(models.Model):
    ...
    day = models.ForeignKey(WorkingDay, on_delete=models.CASCADE, related_name='standard_slots_days')
    slot = models.ForeignKey(TimeSlot, on_delete=models.CASCADE, related_name='standard_slots_time')
...
def event_img(instance, filename):
    upload_to = 'Images/'
    ext = filename.split('.')[-1]
    # get filename
    filename = 'Event_Pictures/{}.{}'.format(instance.pk, ext)
    return os.path.join(upload_to, filename)
...
class Event(models.Model):
    ...
    event_image = models.ImageField(upload_to=event_img, verbose_name="Event Image")
    description = models.TextField(max_length=500, blank=True)
